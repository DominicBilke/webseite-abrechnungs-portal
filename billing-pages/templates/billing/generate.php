...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted"><?= $localization->get('create') ?> <?= $localization->get('new') ?> <?= $localization->get('invoice') ?> <?= $localization->get('for') ?> <?= $localization->get('client') ?></p>
        </div>
        <div>
            ...
            <div class="card shadow">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">
                        ...
                        <?= $localization->get('new') ?> <?= $localization->get('invoice') ?>
                    </h6>
                </div>
                <div class="card-body">
                    <form method="POST" action="/billing/generate" id="invoiceForm">
                        <div class="row">
                            <!-- Invoice Information -->
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="status" name="status">
                                        <option value="draft"><?= $localization->get('draft') ?></option>
                                        <option value="sent"><?= $localization->get('sent') ?></option>
                                        <option value="paid"><?= $localization->get('paid') ?></option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    <label for="client_id" class="form-label">
                                        <?= $localization->get('select_client') ?>
                                    </label>
                                    <select class="form-select" id="client_id" name="client_id">
                                        <option value=""><?= $localization->get('select_client') ?></option>
                                        <?php foreach ($companies as $company): ?>
                                            <option value="<?= $company['id'] ?>" <?= $selected_client == $company['id'] ? 'selected' : '' ?>>
                                ...
                                <div class="card border-0 bg-light">
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-6 text-end">
                                                <strong class="text-success fs-5" id="grandTotal">0.00 €</strong>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
document.getElementById('client_id').addEventListener('change', function() {
    ...
    if (selectedOption.value) {
        ...
        document.getElementById('client_email').value = 'client@example.com';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (items.length === 0) {
            alert('<?= $localization->get('at_least_one_item_required') ?>');
            isValid = false;
        }
    ...
    function saveDraft() {
        ...
        localStorage.setItem('invoice_draft_new', JSON.stringify(draftData));
    ...
    const savedDraft = localStorage.getItem('invoice_draft_new');
    if (savedDraft) {
